...
| 领域           | 已有通道                                                                                                                                                              | 用途                                 |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| 店铺           | shop:list、shop:addAccount、shop:removeAccount、shop:switchView、shop:exitView、shop:start、shop:stop、shop:setAutoReply、shop:takeover、shop:release、shop:sendReply | 店铺、切换、启动、AI、人工和文字回复 |
...
| UI-SIDEBAR-001         | P2     | 已完成     | 导航入口映射 12 个设置模块；店铺菜单、未读、告警、登录态可见                                                   |
...
开发机运行时验收还需在 `npm run electron:start` 启动并开放 9222 后执行 `node scripts/dev/workspace-smoke.cjs`、`node scripts/dev/interaction-smoke.cjs`、`node scripts/dev/management-smoke.cjs` 和 `node scripts/dev/readonly-ipc-smoke.cjs`。这些脚本检查布局、导航、店铺选择、全局搜索、管理中心 12 个模块和查询型 IPC；它们不发送消息、不修改店铺设置。
